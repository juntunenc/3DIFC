function [volThresh,vol] = getVolThresh(img)
...
ground_truth_vol = 4/3*3.14*5^3; %um^3

for i = 1:thresh_res

    vol_thresh(i) = i / thresh_res;
    vol_mask = zeros(padd0,padd0,padd0);
    vol_mask(vol_norm>vol_thresh(i))= 1;

    vols(i) = sum(vol_mask,"all")*vox_size^3; %um^3

end

[~, min_index] = min(abs(ground_truth_vol-vols));
volThresh = vol_thresh(min_index);
vol = vols(min_index);
